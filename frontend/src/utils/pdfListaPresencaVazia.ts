...
interface PDFListaPresencaVaziaProps {
...
export async function gerarPDFListaPresencaVazia({
  ...
  // Salvar PDF
  const dataStr = data ? data.replace(/\//g, '-') : new Date().toISOString().split('T')[0];
  const nomeArquivo = `lista_presenca_vazia_${capacitacao.id}_${dataStr}.pdf`;
