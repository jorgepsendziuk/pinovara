...
interface ValidacaoCapacitacaoReadOnlyProps {
  capacitacaoId: number;
}

const ValidacaoCapacitacaoReadOnly: React.FC<ValidacaoCapacitacaoReadOnlyProps> = ({ capacitacaoId }) => {
  ...
  const carregarDados = async () => {
    try {
      ...
      setDados({
        ...
        validacao_usuario_nome: nomeValidador ?? capacitacao.validacao_usuario_nome ?? null,
      });
    } catch (error) {
  ...
  return (
    <div style={{ padding: '0 4px' }}>
      <p style={{ marginBottom: '1rem', fontSize: '0.9rem', color: '#64748b' }}>
        Acompanhe o status e o histórico da validação desta capacitação. A edição é feita por coordenadores na lista de capacitações.
      </p>
...
export default ValidacaoCapacitacaoReadOnly;
