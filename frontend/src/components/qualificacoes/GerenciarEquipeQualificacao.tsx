...
import { Users, Plus, XCircle, Loader2 } from 'lucide-react';

interface GerenciarEquipeQualificacaoProps {
  qualificacaoId: number;
  qualificacao: Qualificacao | null;
}

function GerenciarEquipeQualificacao({ qualificacaoId, qualificacao }: GerenciarEquipeQualificacaoProps) {
  ...
  // Verificar se pode gerenciar equipe (criador ou admin)
  const canGerenciarEquipe = qualificacao && (qualificacao.created_by === Number(user?.id) || isAdmin());

  const carregarEquipe = useCallback(async () => {
    if (!qualificacaoId) return;
    setCarregandoEquipe(true);
    try {
      const equipe = await qualificacaoAPI.listTecnicos(qualificacaoId);
  ...
  const carregarTecnicosDisponiveis = useCallback(async () => {
    if (!qualificacaoId || !canGerenciarEquipe) return;
    setCarregandoTecnicosDisponiveis(true);
    try {
      // Buscar técnicos disponíveis via API específica para técnicos
  ...
  }, [qualificacaoId, canGerenciarEquipe, equipeTecnica]);

  const handleAdicionarTecnico = async () => {
    if (!qualificacaoId || !canGerenciarEquipe || !tecnicoSelecionado || typeof tecnicoSelecionado !== 'number') {
      return;
    }
    setAdicionandoTecnico(true);
    try {
      await qualificacaoAPI.adicionarTecnico(qualificacaoId, tecnicoSelecionado);
  ...
  const handleRemoverTecnico = async (idTecnico: number) => {
    if (!qualificacaoId || !canGerenciarEquipe) return;
    const confirmacao = window.confirm('Tem certeza que deseja remover este técnico da qualificação?');
    if (!confirmacao) return;
    setRemovendoTecnicoId(idTecnico);
    try {
      await qualificacaoAPI.removerTecnico(qualificacaoId, idTecnico);
      await carregarEquipe();
      await carregarTecnicosDisponiveis();
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      {qualificacao?.criador && (
        <div style={{
          ...
          <span style={{ color: '#111827', fontSize: '14px' }}>{qualificacao.criador.name}</span>
          {qualificacao.criador.email && (
            <span style={{ color: '#6b7280', fontSize: '13px' }}>{qualificacao.criador.email}</span>
          )}
        </div>
      )}
      ...
      <div>
        ...
        {!carregandoEquipe && equipeTecnica.length === 0 && (
          <p style={{ color: '#6b7280', fontSize: '14px', margin: 0 }}>
            Nenhum técnico adicional possui acesso a esta qualificação.
          </p>
        )}
...
export default GerenciarEquipeQualificacao;
