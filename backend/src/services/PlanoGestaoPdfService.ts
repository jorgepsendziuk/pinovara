...
class PlanoGestaoPdfService {
  ...
  private ensureSpace(doc: any, neededHeight: number) {
  ...
  private renderPlano(doc: any, plano: { titulo: string; tipo: string; grupos: { nome: string | null; acoes: AcaoCompleta[] }[] }, indice: number) {
    if (indice > 0) {
      doc.addPage();
      doc.y = 60;
    }
    ...
    plano.grupos.forEach((grupo) => {
      ...
      grupo.acoes.forEach((acao) => {
        ...
        const tituloAcao = acao.acao || (acao as any).acao_modelo || 'Ação sem descrição';
  ...
  async gerarPdfPlanoGestao(idOrganizacao: number): Promise<PassThrough> {
    ...
    try {
      ...
      const totalAcoesAtivas = planosFiltrados.reduce((total, plano) =>
        total + plano.grupos.reduce((acc, grupo) => acc + grupo.acoes.length, 0), 0);
