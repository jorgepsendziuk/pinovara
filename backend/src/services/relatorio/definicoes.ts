...
import { renderizarSubarea, renderizarTabelaPraticas } from './renderizacaoPDF';

export const TODAS_AREAS = [
 ...
 */
export function renderizarTodasAreasGerenciais(doc: any, org: any) {
  TODAS_AREAS.forEach((area, index) => {
    ...
    // Renderizar subareas
    area.sub.forEach((subarea: any) => {
      // Renderizar subárea
      renderizarSubarea(doc, subarea.nome);
      
      // Montar práticas com dados
      const praticas = subarea.p.map((p: any) => ({
        numero: p.n,
        titulo: p.t,
      ...
      renderizarTabelaPraticas(doc, praticas);
    });
  });
}
